...
execute as @e[type=!#sourcecraft.5:no_health,tag=!healthindications.ignore,tag=!global.ignore] if score #healthindications.gametime sourcecraft.temp = @s healthindications.health_bar.despawn run function healthindications.2:health_bar/end_health_bar
